...
## D46 — LATWAN R1A (MON-20): métricas reales en fallos de smoke, 401 por señal estructurada, evidencia durable sanitizada y adaptación declarativa `text_json` de las tres rutas libres
...
1. **Métricas reales en fallos de smoke (L-01).** `KeyRotatingProvider` y
   `ProviderChain` exponen `metrics_snapshot()`; `_run_one` toma un snapshot
   ANTES del smoke y persiste el DELTA en el artefacto, tanto en éxito como en
   fallo. `_smoke_failed` jamas vuelve a fijar `retries/rotations/quarantined`
   en cero por omisión. Nuevo contador `transient_retries_executed`: un retry
   REAL ejecutado por reintento de infraestructura (el dedupe
   `turns_transient_affected` solo dice si el turno se vio afectado, no
   cuántas veces se reintentó). El `retries` del artefacto sale de ese
   contador. Canarios: North (401 credential → quarantined=1) y Ling
   (503 ×3 → retries=2).
...
3. **Evidencia durable sanitizada (L-03).** `MatrixModelResult` (y
   `BenchmarkResult`, que la alimenta en la fase de batalla) agregan
   `failure_stage` (`smoke`/`battle`), `http_status` (cuando existe) y
   `provider_error_code` — este último SOLO desde campos estructurados
   permitidos (`error.code`, `error.details[].reason`), nunca desde texto
   libre. Prohibido persistir mensajes, URLs, headers, bodies completos,
   nombres/valores de variables o secretos (canario con URL+clave en el
   mensaje crudo). Los campos tienen default `None` para que los state
   files viejos sigan cargando.
...
(19 rojos → 101 verdes en la suite focal; 633 verdes + 68 skipped en
`apps/agent` completo, los skipped son integración con DB/Showdown — esta
ronda es offline). Mutaciones dirigidas verificadas: revertir los zeros
fijos de `_smoke_failed` (rojo North + 503), revertir el 401 incondicional
(rojo 401-fatal), extraer `provider_error_code` de texto libre (rojo
sanitización) y revertir cada ruta a `json_schema` (rojo canario de rutas).
Secret scan compartido (L-03 previo) sigue en verde sobre `evals/**`.
