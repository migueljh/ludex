...
# --- F2-09 (MON-14): adapter de ejecucion y resolucion por turno ----------


def test_execute_action_traduce_a_order_y_devuelve_none_fuera_de_mascara():
    """El adapter explicito del resultado del grafo a la ejecucion de
    poke-env: traduce el action canonico al BattleOrder del mapa capturado, y
    devuelve None (que el caller convierte en RuntimeError) para cualquier
    action fuera de la mascara."""
    order = FakeOrder(mid="tackle")
    orders = {
        tuple(sorted({"kind": "move", "id": "tackle"}.items())): order,
    }

    assert client_module.execute_action(
        {"kind": "move", "id": "tackle"}, orders
    ) is order
    assert client_module.execute_action(
        {"kind": "move", "id": "surf"}, orders
    ) is None
    assert client_module.execute_action(
        {"kind": "switch", "species": "pikachu"}, orders
    ) is None


async def test_el_caller_ejecuta_despues_de_decide_en_orden(monkeypatch):
    """Correspondencia end-to-end (F2-09): el grafo corre
    resolve_provider → parse_state → retrieve_context → calc_damage → decide
    y el adapter de ejecucion se aplica DESPUES, dentro de choose_move, sobre
    el resultado del grafo."""
    import ludex_agent.graph.workflow as workflow_module

    real_calc = workflow_module.calc_damage

    async def recording_calc(state, calculator):
        events.append("calc_damage")
        return await real_calc(state, calculator)

    monkeypatch.setattr(workflow_module, "calc_damage", recording_calc)
...
async def test_la_metadata_efectiva_cambia_por_turno_hasta_el_step():
    """BLOQUEANTE 7 (F2-09): provider/model efectivos se resuelven por turno
    y sobreviven hasta el step canonico: dos decisiones del mismo tag con
    selecciones activas distintas dejan steps con metadata distinta, sin
    recompilar el grafo."""
    ...
    class TurnResolver:
        def __init__(self):
            self.turnos = [
                ResolvedProvider("google", "gemini-2.5-flash", TurnProvider("google", "gemini-2.5-flash")),
                ResolvedProvider("kimi", "kimi-k2.6", TurnProvider("kimi", "kimi-k2.6")),
            ]
            self.index = 0
    ...
    graph = build_decision_graph(Calculator(), TurnResolver(), DecisionMetrics(), Repository())
    player = _player(decision_graph=graph, decision_budget_seconds=5)
    with patch.object(
        client_module, "serialize_battle",
        lambda b: {
